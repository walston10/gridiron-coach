...
export type CardCategory = 'OFFENSIVE' | 'DEFENSIVE' | 'SPECIAL_TEAMS' | 'DIRTY';
...
/**
 * Special Teams cards handle kicks, punts, and returns.
 * Used on 4th down and after scores.
 * Generated by ST player and returner designation.
 */
export type SpecialTeamsPlayType =
  // Kicking
  | 'FIELD_GOAL'          // Attempt FG - success based on distance + KAC
  | 'EXTRA_POINT'         // PAT attempt (near automatic, but can be blocked)
  | 'KICKOFF'             // Standard kickoff
  | 'KICKOFF_DEEP'        // Boom it for touchback (KPW)
  | 'KICKOFF_ONSIDE'      // Risky recovery attempt
  | 'KICKOFF_SQUIB'       // Short kick to avoid returner
  // Punting
  | 'PUNT'                // Standard punt
  | 'PUNT_COFFIN_CORNER'  // Directional punt (KAC)
  | 'PUNT_BOOMING'        // Max distance (KPW)
  | 'PUNT_POOCH'          // High hang time, short distance
  // Returns
  | 'KICK_RETURN'         // Return a kickoff
  | 'PUNT_RETURN'         // Return a punt
  | 'FAIR_CATCH'          // Safe, no return
  // Fakes (risky, high reward)
  | 'FAKE_FG_PASS'        // Fake FG, pass attempt
  | 'FAKE_FG_RUN'         // Fake FG, run attempt
  | 'FAKE_PUNT_PASS'      // Fake punt, pass attempt
  | 'FAKE_PUNT_RUN';      // Fake punt, run attempt

/** A special teams card */
export interface SpecialTeamsCard {
  id: string;
  category: 'SPECIAL_TEAMS';
  name: string;                     // "50-Yard Bomb", "Coffin Corner"
  description: string;
  playType: SpecialTeamsPlayType;
  rarity: CardRarity;

  // Core stats (vary by play type)
  successChance: number;            // Base success % (0-100)
  distance?: number;                // For FG: max distance; for kicks: expected yards
  hangTime?: number;                // For punts: affects coverage (0-100)
  returnYards?: number;             // For returns: expected return yards

  // Risk/Reward
  blockChance: number;              // Chance of block (0-100)
  fumbleChance: number;             // For returns: fumble risk (0-100)
  bigReturnChance: number;          // Chance of big return/TD (0-100)

  // Fake play stats (only for FAKE_* types)
  fakeSuccessChance?: number;       // If they fall for it
  fakeYardsGained?: number;         // Yards if successful
  fakeTurnoverRisk?: number;        // Risk if it fails

  // Situational modifiers
  situationBonuses: SituationBonus[];

  // What affects this card
  affectedByKPW: boolean;           // Kick power matters
  affectedByKAC: boolean;           // Kick accuracy matters
  affectedByCLT: boolean;           // Clutch kicking matters
  affectedByCOV: boolean;           // Coverage unit matters

  // For returns: affected by returner's ratings
  affectedByReturnerSpeed?: boolean;
  affectedByReturnerCarrying?: boolean;

  // Source
  generatedBy: string;              // ST player ID or returner ID
  playerBonus?: string;             // "McPherson: +5 yards FG range"
...
export type Card = OffensiveCard | DefensiveCard | SpecialTeamsCard | DirtyCard;
...
export interface CardPlayResult {
  ...
  touchdown: boolean;
  safety: boolean;
  fieldGoalMade?: boolean;          // For FG attempts
  fieldGoalBlocked?: boolean;       // For FG/XP attempts
  puntBlocked?: boolean;            // For punt attempts
...
export interface CardTemplate {
  ...
  playType: OffensivePlayType | DefensivePlayType | SpecialTeamsPlayType | DirtyPlayType;
...
export function isSpecialTeamsCard(card: Card): card is SpecialTeamsCard {
  return card.category === 'SPECIAL_TEAMS';
}
...
export const CATEGORY_COLORS: Record<CardCategory, string> = {
  ...
  SPECIAL_TEAMS: '#F59E0B', // Amber (special teams)
